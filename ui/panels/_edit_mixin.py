...
Mixin 复用: 拖动方框 / 右下角手柄缩放方框 / 拖动文字标注。开启编辑模式后叠加网格与
坐标提示。坐标全部归一化(0~1)。
...
class LayoutEditMixin:
    ...
    def _hit_test(self, pos):
        """返回 (mode, kind, key)。优先级: 缩放手柄 > 方框体 > 标注。"""
    ...
    def mouseMoveEvent(self, e):
        ...
        if self._drag_mode is None:
            ...
            self.setCursor({"resize": Qt.CursorShape.SizeFDiagCursor,
                            ...
                            "label": Qt.CursorShape.PointingHandCursor}.get(
        ...
        if self._drag_mode == "move":
            ...
            obj.x = _clamp(_snap(nx), 0.0, 1.0 - obj.w)
            obj.y = _clamp(_snap(ny), 0.0, 1.0 - obj.h)
        else:  # resize
    ...
    def draw_edit_overlay(self, p):
        ...
        p.drawText(QRectF(ox, oy - 2, w, 14), Qt.AlignmentFlag.AlignRight,
                   "编辑: 拖方框移动·拖右下角改大小·拖圆点移文字, 完成点[导出布局]")
